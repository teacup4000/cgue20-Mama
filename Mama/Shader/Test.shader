...
// function prototypes
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, int i);
...
void main()
{
	...
	for (int i = 0; i < NR_POINT_LIGHTS; i++)
	{
		result += CalcPointLight(pointLights[i], norm, FragPos, i);
	}
	result += CalcShadow(lightPos, n, FragPos);
...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, int i)
{
	...
	vec3 diffuseLight = texture(material.diffuse, TexCoords).rgb;
	vec3 specularLight = light.specular * texture(material.specular, TexCoords).rgb;
	vec3 ambientLight = (light.ambient * diffuseLight) * brightness;

	vec3 lightDir = normalize(lightPos - fragPos);
	vec3 viewDir = normalize(viewPos - fragPos);
	// diffuse shading
	float diff = max(dot(lightDir, normal), 0.0);
	diffuseLight = light.diffuse * diff * diffuseLight;
...
vec3 CalcShadow(vec3 lightPos, vec3 normal, vec3 fragPos)
{
	...
	// diffuse
	vec3 lightDir = normalize(TangentLightPos[7] - TangentFragPos[7]);
	float diff = max(dot(lightDir, normal), 0.0);
	vec3 diffuse = diff * lightColor;
	// specular
	vec3 viewDir = normalize(TangentViewPos[7] - TangentFragPos[7]);
